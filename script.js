...
function seasonFeaturedHTML(p,idx){
  ...
  return '<div class="season-featured"><div class="season-feat-img">'+imgBlock+'</div><div class="season-feat-info"><span class="season-tag">Edición limitada · '+matsLabel(p.materiales)+'</span><h3 class="season-feat-title">'+p.nombre+'</h3><p class="season-feat-desc">'+p.descripcion+'</p><div class="season-feat-price">'+p.precio+'</div><div style="display:flex;gap:.75rem;align-items:center;"><button class="btn-filled" onclick="addToCart(\''+safeQ(p.nombre)+'\',\''+safeQ(p.precio)+'\',\''+safeQ(p.imgPrincipal||'')+'\','+idx+')">Añadir al carrito</button><button class="modal-fav-btn fav-btn" data-fav-name="'+safeQ(p.nombre)+'" onclick="window.toggleFav&&toggleFav({name:\''+safeQ(p.nombre)+'\',price:\''+safeQ(p.precio)+'\',img:\''+safeQ(p.imgPrincipal||'')+'\'})">🤍</button></div></div></div>';
}
